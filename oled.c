...
void oledRepeatByte( BYTE b, int repeat, unsigned char page, unsigned char col)
{
	...
	for(i = 0; i < repeat; ++i)
	{
		oledWriteChar1x(b,page,col+i);
	}
}

void oledPrintRectangle(unsigned char page, unsigned char col)
{
	int i;
	page = page + 0xB0;
	for (i = 0; i < 24; ++i)			// print top horizontal line
		oledWriteChar1x(0x2D,page, col+i);

	for (i = 0; i < 24; ++i)			// print bottom horizontal line
		oledWriteChar1x(0x2D,page + 4,col+i);

	for (i = 0; i < 5; ++i)			// print vertical line
	{
		oledWriteChar1x(0x7C,page +i,col);
		oledWriteChar1x(0x7C,page +i,col + 25);
	}
}
...
void FillInverseDisplay(unsigned char data, unsigned char page,int pageOffset, unsigned char col, int colOffset)
{
	unsigned char i,j;
	page = page + 0xB0;
	for(i = page;i<= page + pageOffset;i++)			// Go through all 8 pages
	{
		WriteCommand(i);		// Set page
		//WriteCommand(0x00+OFFSET);		// Set lower column address
		...
		for(j=col;j<col + colOffset;j++)			// Write to all 132 bytes
		{
			WriteData(data);
		}
	}
...
ROM BYTE g_pucFont[95][5] =
{
    ...
    { 0x20, 0x10, 0x08, 0x10, 0x20 }, // }   0X7D	// for ^ character
    { 0x06, 0x09, 0x00, 0x09, 0x06 }, // ~   0X7E  // for celcius icon
};